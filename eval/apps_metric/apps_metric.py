...
class apps_metric(evaluate.EvaluationModule):
    ...
    def _compute(self, predictions, k_list=[1, 5, 10], count_errors=True, split="test", level="all", debug=False):
        """Returns the scores"""
        metrics, results = compute_metrics(predictions, k_list=k_list, count_errors=count_errors, split=split, level=level, debug=debug)
